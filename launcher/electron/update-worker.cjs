...
function updateMac(job) {
  const sourceExecutable = path.join(job.source, "Contents", "MacOS", "Codex Web GPT Secure");
  ...
  requireFile(path.join(next, "Contents", "MacOS", "Codex Web GPT Secure"), "Copied macOS launcher");
